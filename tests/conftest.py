...
tmp dir by the time conftest is loaded — so we force-set both
``COVERAGE_PROCESS_START`` and ``COVERAGE_FILE`` and drop a
``.pth`` shim into site-packages that calls
``coverage.process_startup()`` for child interpreters.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_datetime_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
